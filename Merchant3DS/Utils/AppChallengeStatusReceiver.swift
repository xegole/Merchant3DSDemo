...
class AppChallengeStatusReceiver: ChallengeStatusReceiver {
    ...
    init(view: StatusView, dsTransId: String?, transactionId: String?) {
    ...
    func completed(completionEvent: CompletionEvent) {
        if TransactionStatus.AuthenticationSuccessful.rawValue == completionEvent.getTransactionStatus() {
            if let transactionId = transactionId {
                self.statusView?.showSuccessScreen(by: transactionId)
            } else if let dsTransId = dsTransId {
                self.statusView?.showSuccessScreen(with: "The transaction was successful\ndsTransID: \(dsTransId)")
            }
        } else {
