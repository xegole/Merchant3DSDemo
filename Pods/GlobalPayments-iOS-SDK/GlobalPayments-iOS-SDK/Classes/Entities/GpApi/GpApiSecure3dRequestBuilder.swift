...
struct GpApiSecure3dRequestBuilder: GpApiRequestData {
    ...
    private func initiateAuthenticationData(_ builder: Secure3dBuilder, _ config: GpApiConfig?) -> JsonDoc {
        ...
        let threeDS = JsonDoc()
            ...
            .set(for: "preference", value: builder.challengeRequestIndicator?.mapped(for: .gpApi) ?? "NO_PREFERENCE" )
            .set(for: "message_version", value: builder.messageVersion)
        
        let mobileData = JsonDoc().set(for: "application_reference", value: builder.mobileData?.applicationReference)
            .set(for: "sdk_interface", value: builder.mobileData?.sdkInterface?.mapped(for: .gpApi))
            .set(for: "sdk_ui_type", value: builder.mobileData?.sdkUiTypes?.map({ type in
                type.rawValue.uppercased()
            }))
            .set(for: "ephemeral_public_key", doc: builder.mobileData?.ephemeralPublicKey)
            .set(for: "maximum_timeout", value: "\(builder.mobileData?.maximumTimeout ?? 10)")
